
import { NextResponse } from 'next/server';
import { db } from '@/lib/db';

export async function GET() {
  try {
    const medications = await db.getAllMasterMedications();
    return NextResponse.json(medications);
  } catch (error) {
    console.error("Error fetching master medications:", error);
    return NextResponse.json({ message: 'An error occurred' }, { status: 500 });
  }
}

export async function POST(request: Request) {
  try {
    const { name } = await request.json() as { name: string };

    if (!name) {
         return NextResponse.json({ message: 'Name is required' }, { status: 400 });
    }

    const newMed = await db.createMasterMedication({ name });

    return NextResponse.json(newMed, { status: 201 });

  } catch (error) {
    console.error("Error creating master medication:", error);
    const e = error as Error;
    return NextResponse.json({ message: e.message || 'An error occurred' }, { status: 500 });
  }
}
